...
export function ClaimView({ slug, roundId }: { slug: string; roundId?: string }) {
  ...
  const network = view.claim?.network ?? "devnet";
        ...
        {status === "sent" ? (
          <div role="status" className="stack center" style={{ alignItems: "center", gap: 12 }}>
            ...
            <div className="row wrap" style={{ gap: 10, justifyContent: "center" }}>
              <p className="body">{prize.name} prize token was sent to your wallet on Solana {network}.</p>
          ...
          <div className="stack" style={{ padding: "22px 28px 24px", gap: 20 }}>
            ...
            {status === "pending" ? (
              ...
              <div className="stack" style={{ gap: 12 }}>
                ...
                {view.claim?.tx && (
                  <a className="btn btn-ghost-night" href={explorer(view.claim.tx, network)} target="_blank" rel="noreferrer">
                    View transaction
